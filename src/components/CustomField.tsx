...
const CustomField = ({ field, errors }: any) => {
  ...
  const error = errors instanceof Array && errors?.filter((e: { id: any; }) => e.id === field.id)[0];
  const errorMessage = error && errors instanceof Array && errors?.filter((e: { id: any; }) => e.id === field.id)[0].message;
